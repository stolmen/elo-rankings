...
function getapi(dbService, lightService) {
	return {
		getRankings: async function () {
			const players = await dbService.getCollection('players')
			return players.find().sort({elo: -1}).toArray()
		},

		getHistory: async function () {
			const history = await dbService.getCollection('history')
		...
		resolveGameNvN: async function (query) {
			...
			const winnerUpdates = winnerDocs.map(doc => {
				return players.update({_id: dbService.mongodb.ObjectID(doc._id)}, {
		...
		undoLastGame: async function () {
			...
			const historyUpdate = history.deleteOne({_id: dbService.mongodb.ObjectID(lastGame._id)})
...
module.exports = {getApiService: getapi};

...
function getSlackService(apiService, slackApiToken, slackChannel) {
	return {
		start: async function () {
			const web = new WebClient(slackApiToken)
			const res = await web.conversations.list()
			const channels = res.channels
			const botChannel = channels.find(c => c.name === slackChannel)
			if (!botChannel) {
			...
			rtm.on('message', function (message) {
				...
				if (message.text.startsWith('!')) {
					...
					const command = this.commands[commandArgs[0]]
					if (!command) {
						return
					}
		...
		commands: {
			...
			'h2h': {
				...
				handler: async (sendMessage, args) => {
					...
					const h2hstats = await apiService.h2h({player1, player2});
			...
			'newplayer': {
				...
				handler: async (sendMessage, args) => {
					...
					try {
						await apiService.newPlayer({name: args[0]})
			...
			'rank': {
				...
				handler: async (sendMessage, args) => {
					try {
						let rankings = await apiService.getRankings()
			...
			'whoops': {
				...
				handler: async (sendMessage, args) => {
					try {
						const undone = await apiService.undoLastGame()
...
module.exports = { getSlackService };
